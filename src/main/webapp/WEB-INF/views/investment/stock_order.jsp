	...
	<div class="container">
				...
				<c:forEach var="order" items="${saleOrders}">
					<c:if test="${order.stock.stockId == stock.stockId}">
						<tr>
							...
							<td>
								<!-- 구매 버튼 -->
								<form action="/spike.com/listing/buy" method="post"
									style="display: inline;">
									<input type="hidden" name="${_csrf.parameterName}"
										value="${_csrf.token}" /> <input type="hidden"
										name="buyerAccountId" value="${securitiesAccount.accountId}" />
									<input type="hidden" name="listingId" value="${order.id}" /> <input
										type="hidden" name="quantity" value="${order.quantity}" />
									<button type="submit">구매</button>
								</form>
							</td>
							<td>
								<!-- 취소 버튼 --> <c:if
									test="${order.seller.accountId == securitiesAccount.accountId}">
									<form action="/spike.com/listing/cancel" method="post"
										style="display: inline;">
										<input type="hidden" name="${_csrf.parameterName}"
											value="${_csrf.token}" /> <input type="hidden"
											name="sellerAccountId" value="${securitiesAccount.accountId}" />
		...
		<div class="order-section">
			...
			<form id="sell-form" class="order-form"
				...
				<label>판매 가격</label> <input type="text" name="price"
					placeholder="판매 가격 입력"> <label>판매 수량</label> <input
					type="number" name="quantity" min="1"
					max="${ownedQuantity - listedQuantity}" placeholder="판매 수량 입력">
