...
<body>
  ...
  <div class="container">
    ...
    <section class="account-balance">
      <h3>예수금 보유량</h3>
      <p>예수금: <strong>${availableBalance} 원</strong></p>
    </section>
      ...
      <table>
        ...
        <tbody>
          <c:forEach var="holding" items="${stockHoldings}">
            <tr>
              <td>${holding.stock.companyName} (${holding.stock.tickerSymbol})</td>
              <td>${holding.quantity} 주</td>
              <td>${holding.stock.currentPrice} 원</td>
              <td>
                <c:set var="stockValue" value="${holding.quantity * holding.stock.currentPrice}" />
                ${stockValue} 원
              </td>
            </tr>
          </c:forEach>
        ...
        <tbody>
          <c:forEach var="transaction" items="${transactionList}">
            <tr>
              ...
              <td>${transaction.stock.companyName} (${transaction.stock.tickerSymbol})</td>
              <td>
                <c:choose>
                  <c:when test="${transaction.buyer.accountId == currentAccountId}">
                    구매
                  </c:when>
                  <c:otherwise>
                    판매
