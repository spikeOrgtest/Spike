...
<body>
	...
	<c:if test="${not empty errorMessage}">
		<%-- not empty requestScope.errorMessage or  --%>
		<script>
        	alert("${errorMessage}");
			//javascript if 조건에 EL을 사용할 때 if(${needRedirection}) 으로 사용하면 판정이 불가능한 현상 발생(이론상 if(true)로 판정해야 하는데 문법오류처럼 동작)
			//에러 종류에 따른 동작 제어 방법 찾아야 할 듯(별도 에러 페이지 or 핸들러 클래스) -> transfer 메서드에서 발생한 에러는 에러 페이지 제작해서 처리해봄(transfer_errorpage.jsp)
					...
					<main>
						...
						<form action="/spike.com/transfer" method="post"
							...
							<section class="transfer-container container">
								...
								<div class="transfer-section mb-5 p-4 border rounded shadow">
												...
												<c:otherwise>
													<c:forEach var="account" items="${accountList}">
														<option value="${account.accountId}"
															...
															data-availableLeft="${account.availableLimit}"
									...
									<div class="row">
										<div class="col">
											<label class="form-label">잔여한도/일일한도금액</label>
											<div id="availableWrapper" style="display: flex;">
											<div class="form-control bg-light" id="availableAmountLeft">
												- 원</div>
						...
						<section class="transfer-history mt-5">
							<h4>
								...
								<p style="font-size: 16px; color: gray;">최대 30일까지의 송금 내역만
									표시됩니다</p>
							</h4>
									...
									<c:when test="${not empty histories}">
										<c:forEach var="history" items="${histories}">
											<li
												...
												style="padding: 5px 15px;"><span
												style="padding: 2px 9px;"><fmt:formatDate
														value="${history.transactionDate}" pattern="yy/MM/dd" /><br>
												<br>
												<fmt:formatDate value="${history.transactionDate}"
														pattern="HH:mm" /> </span> <span>| ${history.name} |</span> <span
												class="badge bg-secondary"
												style="color: gray; font-size: 14px;">-<fmt:formatNumber
														value="${history.amount}" pattern="#,###" /> 원<br>
												<br>
												<fmt:formatNumber value="${history.afterBalance}"
														pattern="#,###" /> 원
											</span></li>
										</c:forEach>
									</c:when>
