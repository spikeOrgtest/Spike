...
public class TransactionServiceImpl implements TransactionService {
	...
	public void transfer(TransferDTO tData) {
		...
		if(fromAccount.getBalance() < tData.getAmount()) throw new IllegalArgumentException("잔액이 부족한데 어떻게 송금하셨죠?");
	...
	public List<TransferHistoryDTO> getRecentTransfers(Long userId) {
		...
		//페이징 위한 pageable 객체(인데 5개만 조회할 목적으로 사용), PageRequest 정적메서드 ofSize로 객체생성 (생성자 팩토리 어쩌구 패턴)
		Pageable pageable = PageRequest.ofSize(5); // == .of(0, 5) 여기선 무조건 5개만 보여줄거라 offset 0
		//pageable 사용해서 조회하면 Page<T> type으로 반환하는것 유의, .getContent()로 꺼내줘야 List<T> 되는데 내부적으로 처리해주는듯??
		List<TransactionDTO> transactions = this.transactionRepo.findTop5ByFromAccount_Owner_UserIdOrderByTransactionDateDesc(userId, pageable);
		...
		/*     // List -> stream으로 변환, stream 내장메서드 map을 사용해서 Transaction->TransactionHistory로 변환, 다시 List로 변환
		 * 	   //List<T> -> stream() -> Stream<T> -> map() -> Stream<R> -> collect() -> List<R>
		return transactions.stream()
				.map(transaction -> TransferHistoryDTO.builder()
						.name(transaction.getToAccount().getOwnerName()) 
						.transactionDate(transaction.getTransactionDate())
						.amount(transaction.getAmount())
						.balanceAfterTransaction(transaction.getFromAccount().getBalance())
						.build()) 
