...
public interface QuizResultService {
		 ...
		 */
		QuizResultDTO handleQuizResult(Long userId, int quizId, char answeredCorrectly);
		 ...
		 */
		boolean canUserAttemptQuizToday(Long userId, String date);
		 ...
		 */
		boolean attemptQuiz(Long userId, int quizId, String userAnswer);
		 ...
		 * @return 오늘 퀴즈를 시도할 수 있으면 true, 아니면 false
		 */
		boolean canAttemptQuizToday(Long userId);
		 ...
		 * 사용자 포인트를 업데이트하는 메소드.
		 * 
		 * 이 메소드는 사용자가 퀴즈를 풀었을 때 얻은 포인트를 업데이트합니다.
		 * 
		 * @param userId 사용자의 ID
		 * @param earnedPoints 획득한 포인트
		 */
