...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests()
				// 인증이 필요한 페이지만 지정
				.antMatchers("/profile", "/dashboard", "/admin", "/spike.com/mypage/main").authenticated() // 인증된 사용자만
																											// 접근할 수 있는
																											// 경로
				...
				.and().logout().logoutUrl("/logout") // 로그아웃 URL
				.logoutSuccessUrl("/") // 로그아웃 후 리디렉션할 URL
				.permitAll() // 로그아웃 페이지도 모두 허용
				.and().sessionManagement().sessionCreationPolicy(SessionCreationPolicy.IF_REQUIRED) // 세션 관리 정책
