...
public class TransactionController {
	...
	public String transfer_ok(Long fromAccountId, String toAccount, long amount, String memo, String accountPassword,
		...
		try {
			...
			String password = fromAcc.getAccount_password();
			String ownerName = owner.getName();
			String ownerStatus = owner.getStatus();
			//계좌 주인 이름, 상태 구하는 코드 통합 -> 아래 코드는 삭제예정
			//받는 사람의 이름 조회
			//받는 사람 상태 조회(shield 때문에 필요)
			//String password = this.transService.getAccount(fromAccountId).getAccount_password();
			...
			TransferDTO tData = new TransferDTO(fromAccountId, toAccount, amount, memo, accountPassword, ownerName, ownerStatus);
			...
			return "redirect:/spike.com/transfer";
			/*PrintWriter out;  //컨트롤러에서 직접 <script>를 출력하는 방법(수업소스), 정상 동작하지만 날코딩 느낌
			try {
	...
	//PRG패턴의 Get부분, 히스토리에 post 요청이 남지 않아서 뒤로가기 눌러도 @Postmapping("transfer_ok") 요청이 발생하지 않는 원리
